...
interface ICardHeaderProps {
  ...
  showViewAll?: boolean; // Optional prop
}
...
// --- Initial structure for Stats Cards (outside component) ---
...
// --- NEW Department Overview Component ---
...
// --- NEW Recent KPI Updates Component ---
...
// --- Initial structure for Line Chart Data (outside component) ---
  ...
  datasets: [
    {
      ...
      // Made-up data representing staff distribution
...
const Dashboard: React.FC = () => {
  ...
  // States for our master data lists
  ...
  React.useEffect(() => {
    // This simulates fetching data. We can use our imported constants.
    setAllKpis(KPIs);
  ...
  // 2. Recalculate stats whenever any source data changes
  React.useEffect(() => {
  ...
  // 3. Recalculate line chart data whenever KPIs change
  React.useEffect(() => {
    ...
    // Create a cumulative count for a steadily rising line graph
  ...
  //     <span>{title}</span>
  //     {showViewAll && <button>View All</button>}
  //   </div>
  // );
  ...
  //     <span>{title}</span>
  //     {showViewAll && (
  //       <button onClick={() => navigate("/department-management")}>
  //         View All
  //       </button>
  //     )}
  //   </div>
