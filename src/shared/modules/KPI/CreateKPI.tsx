...
const createKPI: React.FC = () => {
  ...
  // 1. Find the index of the current active tab. This is crucial for navigation.
  const activeTabIndex = tabs.findIndex((tab) => tab.id === activeTab);

  // 2. Create handler functions for Next and Previous buttons.
  const handleNext = () => {
    // Check if we are not on the last tab
    if (activeTabIndex < tabs.length - 1) {
      // Get the ID of the next tab and update the state
  ...
  const handlePrevious = () => {
    // Check if we are not on the first tab
    if (activeTabIndex > 0) {
      // Get the ID of the previous tab and update the state
  ...
  return (
    <section>
      ...
      <Card title="">
        {/* The form tag should ideally wrap the content and buttons */}
        <form action="" id="">
          <div className="tabs">
            {tabs.map((tab) => (
              <button
          ...
          {/* This renders the content of the active tab */}
          ...
          >
            <div
              ...
              // id={styles["kpi-controls"]}
            >
              <div>
                {/* 3. Conditionally render the "Previous" button */}
                {activeTabIndex > 0 && (
                  <Button
                    variant="secondary"
                    onClick={handlePrevious} // <-- Add onClick handler
              ...
              <div className="d-flex gap-2">
                {/* 4. Conditionally render the "Next" button */}
                ...
                {/* 5. Conditionally render the final "Create" button */}
                {activeTabIndex === tabs.length - 1 && (
                  <Button
                    variant="success"
                    type="submit" // <-- This is the only button that should submit the form
                  >
                    <Icon iconName="Save" />
                    Create KPI
                  </Button>
